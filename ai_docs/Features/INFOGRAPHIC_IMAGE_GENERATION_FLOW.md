# Infographic Image Generation Flow

This note explains the full flow when a user gives a prompt in Brand Space chat and selects **Infographic** in the Studio Panel, for example:
...
flowchart TD
    ...
    N --> O[Infographic planning prompt]
    O --> P[Planning response with text, sections, chart/table metadata]
    ...
    U --> V[Generated infographic image asset]
    ...
    Z --> AA[Frontend displays infographic image]
```

Schema files are not shown as main boxes in this diagram because they do not perform generation. They only define and validate the request data shape. In simple terms:
...
```text
ChatService detects that the chat prompt plus Studio Panel needs infographic visual generation and routes it to ContentService.generate().
```
...
```text
infographic means one infographic-style visual with sections/modules.
```

The platform changes platform-specific guidance:

```text
Instagram infographic usually needs mobile-first readability, square/portrait-safe composition, and strong visual hierarchy.
LinkedIn infographic usually needs professional/business readability, cleaner structure, and explanation-friendly layout.
```
...
The same infographic orchestrator flow runs for LinkedIn and Instagram. The difference is that platform, size, and export type are passed into planning, final prompt, image generation, asset saving, and export metadata.
...
For infographic, it tries to find:

```text
infographic-like samples
multi-section layouts
comparison/explainer/data layouts
chart/table/module style references
```
...
```text
OpenAI or Anthropic can be used for text planning and structured JSON. This stage prepares the content, metadata, chart/table rules, scene graph, and final image prompt.
```
...
```text
Decides what the infographic is trying to communicate.
```
...
```text
Message strategy is the communication plan before detailed infographic planning.
```

## Step 11: Infographic Planning Prompt
...
```text
Asks the text AI to create structured planning for the infographic.
```

For infographic, it asks for metadata like:
...
```text
Planning prompt asks AI to plan infographic content, sections, data visuals, and visual direction.
```

## Step 12: Chart, Table, and Visual Metadata Validation
...
_infographic_surface_intent
_infographic_surface_enforcement_section
_sanitize_infographic_surface_visual_metadata_fields
```

Purpose:

```text
Orchestrator checks whether chart/table/data visuals are allowed.
```
...
```text
Infographic visuals are based on approved anchors like prompt, proof points, stat highlights, claim-evidence pairs, and research. Orchestrator blocks unsupported fake charts/tables.
```
...
```text
For infographic, text/content used in the image is validated and can be repaired one time before image generation.
```
...
```text
Scene graph is the AI-generated layout map for the new infographic.
```
...
```text
Template is reference. Scene graph is the actual planned structure for the new infographic.
```
...
infographic section plan
approved text
chart/table/data rules
logo-safe area
footer-safe area
...
```text
Final image prompt combines content, brand, template, scene graph, and chart/table safety rules before calling the image provider.
```
...
```text
the full infographic image
small visuals inside the infographic
icons
mini charts or visual modules
cards and sections
...
```text
Small visuals inside the infographic are normally not generated one by one.
They are generated together as part of the final single infographic image.
```
...
## Step 18: Retry and Repair Rules for Infographic

This is important because infographic and carousel behave differently.
...
```text
Infographic content/text can be repaired once before final image generation.
```
...
```text
For infographic, sample similarity retry is 0.
For infographic, final image quality retry is 0.
```
...
prompt adherence
relevance
infographic format quality
visual data coverage
logo/asset usage
```

```text
Brand scoring is not image generation. It evaluates the generated infographic after output exists and stores scoring/debug information.
```
...
```text
Frontend displays the generated infographic using the saved image asset path/URL returned from backend.
```
