...
class KnowledgeAssetRepository(Repository[KnowledgeAsset]):
    # Data-access helper for knowledge asset; services call this class instead of repeating SQLAlchemy filters
    # inline.
    def __init__(self, session: AsyncSession) -> None:
        # Binds KnowledgeAssetRepository to the current async session, giving every query method the same DB
        # transaction context.
    ...
    async def list_by_field(
        ...
        # Returns matching by field records with repository scope applied; services assemble responses from
        # these rows.
...
class TemplateRepository(Repository[Template]):
    # Data-access helper for template; services call this class instead of repeating SQLAlchemy filters inline.
    def __init__(self, session: AsyncSession) -> None:
        # Binds TemplateRepository to the current async session, giving every query method the same DB
        # transaction context.
...
class TemplateMetadataRepository(Repository[TemplateMetadata]):
    # Data-access helper for template metadata; services call this class instead of repeating SQLAlchemy filters
    # inline.
    def __init__(self, session: AsyncSession) -> None:
        # Binds TemplateMetadataRepository to the current async session, giving every query method the same DB
        # transaction context.
        super().__init__(session, TemplateMetadata)

    async def get_by_template(self, template_id: UUID) -> TemplateMetadata | None:
        # Fetches the requested by template record or None, leaving not-found handling to the calling service.
