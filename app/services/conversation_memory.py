...
class ConversationMemoryService:
    ...
    async def _expand_assets_for_content_version(self, asset: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if not content_version_id:
            return [asset]
        assets_repo = getattr(self, "assets", None)
        if assets_repo is None:
            return [asset]
        try:
            assets = await assets_repo.list_by_content(UUID(str(content_version_id)))
        ...
        final_render_assets = [
            ...
            if (getattr(item, "metadata_json", None) or {}).get("render_source") == "ai"
            and (getattr(item, "metadata_json", None) or {}).get("generation_stage") == "final_render"
        ]
        final_render_slide_count = max(
            [
                int((getattr(item, "metadata_json", None) or {}).get("slide_count") or 0)
                for item in final_render_assets
                if str((getattr(item, "metadata_json", None) or {}).get("slide_count") or "").strip().isdigit()
            ],
            default=0,
        )
