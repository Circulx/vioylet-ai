        ...
        <div className="container">
            <div className="w-full space-y-5">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    <MetricCard icon={"/owner/active_tenants.svg"} label="Active Tenants" value={`${displayActiveTenants} / ${displayTotalTenants}`} />
                    <MetricCard icon={"/owner/brand_spaces.svg"} label="Brand Spaces" value={compactNumber(displayBrandSpaces)} />
                    <MetricCard icon={"/owner/users.svg"} label="Total Users" value={compactNumber(displayUsers)} />
                </div>

                <div className="grid gap-4 md:grid-cols-2">
                    <MetricCard icon={"/owner/avg_res.svg"} label="Avg Response Time" value={`${STATIC_RESPONSE_TIME_SECONDS} sec`} />
                    <MetricCard icon={"/owner/brand_score.svg"} label="Brand Alignment Score" value={`${STATIC_BRAND_ALIGNMENT_SCORE}%`} />
                </div>

                <section className="space-y-4">
                    <h2 className="text-xl font-semibold text-[#000000]">Top Risk Tenants</h2>
...
function MetricCard({ icon, label, value }: { icon: string; label: string; value: string }) {
    return (
        <div className="flex min-h-[90px] items-start gap-4 border border-[#E4E7EC] bg-white p-6">
            <div className="flex h-12 w-12 items-center justify-center bg-[#F7F7FB] text-[#6B7280] [&_svg]:h-5 [&_svg]:w-5">
                <Image src={icon} alt={label} width={26} height={26} className="w-6 h-6" />
            </div>
            <div>
                <p className="text-base font-medium text-[#6B7280]">{label}</p>
    ...
    return (
        <Popover open={isOpen} onOpenChange={setIsOpen}>
            <PopoverTrigger asChild>
                <button
                    type="button"
                    className="inline-flex h-12 shrink-0 items-center gap-3 border border-[#D5D8E8] bg-white px-5 text-sm cursor-pointer font-medium text-[#1F2937] shadow-none transition hover:bg-[#FAFAFD]"
                >
            ...
            >
                <div className="space-y-5">
                    <div>
                        <p className="text-base font-semibold leading-6 text-[#2F3342]">Usage window</p>
...
function MonthField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
    return (
        <label className="space-y-2">
            <span className="block text-sm font-medium text-[#666666]">{label}</span>
